...
2. 等待系统完成自动球场标定。视频预览会在约 3.75 秒内抽取最多 6 帧，依次尝试四种后端；系统会检查四边白线、内部单打线、透视形状和连续帧一致性。未通过验证但具备完整几何的结果以黄色虚线显示为“待确认草稿”，通过验证后升级为绿色可信标注。黄色草稿不会用于球速、落点和移动距离等几何统计，可直接拖动角点修正；也可按左上、右上、右下、左下（TL、TR、BR、BL）的顺序重新点选四个角。
