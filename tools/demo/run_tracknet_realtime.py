...
from main import RuntimeConfig, build_track_realtime_runner
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TrackNet realtime inference")
    parser.add_argument("--source", required=True, help='Video path, RTSP URL, or webcam index like "0"')
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "outputs" / "track_realtime"))
...
def main() -> None:
    ...
    config = RuntimeConfig(
        ...
        realtime_save_video=not args.no_save_video,
        realtime_max_frames=args.max_frames,
        save_vis=False,
    )
